...
def native_template() -> dict:
    return {
        ...
        "upgrade": {
            "model": "gpt-5.6-luna",
            "migration_markdown": "template migration",
...
class BridgeTests(unittest.TestCase):
    ...
    def test_build_entry_applies_transport_overrides(self) -> None:
        manifest = {
            "template_slug": "gpt-5.6-sol",
        ...
        self.assertNotIn("tool_mode", result)
        self.assertEqual(result["shell_type"], "shell_command")
        self.assertFalse(result["use_responses_lite"])

    def test_build_entry_clears_inherited_native_upgrade(self) -> None:
        manifest = {
            "template_slug": "gpt-5.6-sol",
    ...
    def test_desktop_probe_uses_temporary_codex_home(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            config = root / "config.toml"
            profile = root / "probe-source.config.toml"
            catalog = root / "catalog.json"
            fake_codex = root / "codex"
            observed = root / "observed-home.txt"
            ...
            fake_codex.write_text(
                ...
                f"pathlib.Path({json.dumps(str(observed))}).write_text(os.environ['CODEX_HOME'])\n"
                "out = pathlib.Path(sys.argv[sys.argv.index('--output-last-message') + 1])\n"
                "out.write_text('CODEX_BRIDGE_OK', encoding='utf-8')\n",
                encoding="utf-8",
            )
            ...
            self.assertEqual(proc.returncode, 0, proc.stderr or proc.stdout)
            self.assertNotEqual(Path(observed.read_text()), root)
            self.assertEqual(config.read_bytes(), before)
            self.assertTrue(json.loads(proc.stdout)["config_unchanged"])
