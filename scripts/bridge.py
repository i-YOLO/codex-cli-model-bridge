...
from deployment import register_deployment_parsers
...
DEFAULT_TRANSPARENT_RUNTIME = DEFAULT_STATE_DIR / "transparent_proxy.py"
...
DEFAULT_SYSTEMD_UNIT = Path("~/.config/systemd/user/codex-cli-model-bridge-transparent-proxy.service").expanduser()
DEFAULT_WINDOWS_TASK = "CodexCliModelBridgeTransparentProxy"
...
def default_proxy_binary() -> Path:
    extra: list[Path] = []
    runtime_state = DEFAULT_STATE_DIR / "runtime.json"
    if runtime_state.exists():
        try:
            value = json.loads(runtime_state.read_text(encoding="utf-8")).get("binary")
            if isinstance(value, str) and value:
                extra.append(Path(value).expanduser())
...
def manifest_paths(selected: set[str] | None = None, state_dir: Path = DEFAULT_STATE_DIR) -> list[Path]:
    paths = sorted((SKILL_DIR / "models").glob("*.json"))
    local = state_dir / "models.d"
    if local.exists():
...
def validate_manifest(data: dict) -> list[str]:
    ...
    if "shell_type" in data and not isinstance(data["shell_type"], str):
        errors.append("shell_type must be a string")
    if "use_responses_lite" in data and not isinstance(data["use_responses_lite"], bool):
        errors.append("use_responses_lite must be a boolean")
...
def build_entry(manifest: dict, templates: dict[str, dict]) -> dict:
    ...
    entry.update(
        {
            ...
            # Native upgrade metadata belongs to the template model's lifecycle.
            # Carrying it into an external route can retire that route and make
            # Codex migrate the conversation to an unrelated native model.
            "upgrade": None,
    ...
    if "shell_type" in manifest:
        entry["shell_type"] = manifest["shell_type"]
    if "use_responses_lite" in manifest:
        entry["use_responses_lite"] = manifest["use_responses_lite"]
    return entry
...
def runtime_invocation(runtime: Path) -> list[str]:
    python = python_executable()
    if Path(python).name.lower() in {"py", "py.exe"}:
        return [python, "-3", str(runtime)]
    return [python, str(runtime)]


def proxy_environment(helper: Path, transparent_url: str, upstream_url: str) -> dict[str, str]:
...
def launch_agent_source(runtime: Path, helper: Path, transparent_url: str, upstream_url: str) -> str:
    payload = {
        "Label": TRANSPARENT_LAUNCH_LABEL,
        "ProgramArguments": runtime_invocation(runtime),
        "EnvironmentVariables": proxy_environment(helper, transparent_url, upstream_url),
        "RunAtLoad": True,
        "KeepAlive": True,
...
def systemd_unit_source(
    runtime: Path,
    helper: Path,
    transparent_url: str,
    upstream_url: str,
) -> str:
    env = proxy_environment(helper, transparent_url, upstream_url)
    invocation = " ".join(subprocess.list2cmdline([part]) for part in runtime_invocation(runtime))
    lines = [
        "[Unit]",
        "Description=Codex CLI Model Bridge transparent proxy",
        "After=network.target",
        "",
        "[Service]",
        "Type=simple",
        *(f'Environment={key}={json.dumps(value)}' for key, value in env.items()),
...
def start_detached_proxy(
    ...
    env = os.environ.copy()
    env.update(proxy_environment(helper, transparent_url, upstream_url))
    kwargs: dict[str, object] = {
        "args": runtime_invocation(runtime),
        "stdin": subprocess.DEVNULL,
...
def start_transparent_proxy(
    ...
    if platform.system() == "Darwin":
        ...
        atomic_write(
            launch_agent_path,
            launch_agent_source(runtime, helper, transparent_url, upstream_url),
            0o600,
        )
        return start_launch_agent(launch_agent_path)
    if platform.system() == "Linux" and shutil.which("systemctl"):
        systemd_unit_path.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
        atomic_write(
            systemd_unit_path,
            systemd_unit_source(runtime, helper, transparent_url, upstream_url),
            0o600,
        )
        ...
        enable_proc = subprocess.run(
            ["systemctl", "--user", "enable", "--now", systemd_unit_path.name],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            check=False,
        )
    ...
    if is_windows() and shutil.which("schtasks"):
        wrapper = runtime.with_name("transparent_proxy.cmd")
        env = proxy_environment(helper, transparent_url, upstream_url)
        wrapper_lines = ["@echo off", *(f'set "{key}={value}"' for key, value in env.items())]
        wrapper_lines.append(subprocess.list2cmdline(runtime_invocation(runtime)))
        atomic_write(wrapper, "\r\n".join(wrapper_lines) + "\r\n", 0o600)
        command = subprocess.list2cmdline([str(wrapper)])
        ...
        if create.returncode != 0:
            return "Task Scheduler failed to register the transparent proxy"
        run = subprocess.run(
        ...
        return None if run.returncode == 0 else "Task Scheduler failed to start the transparent proxy"
    return start_detached_proxy(runtime, helper, transparent_url, upstream_url)
    ...
    for model in models:
        ...
        try:
            ...
            try:
                ...
                response_payload = {}
            answer = response_output_text(response_payload)
            delivered = "CODEX_MULTI_AGENT_OK" in answer
            ok = response.status == 200 and delivered
            results[model] = {
                "ok": ok,
                "status": response.status,
                "task_delivered": delivered,
                "elapsed_seconds": round(time.monotonic() - started, 3),
                "error": None if ok else "agent_message task payload was not delivered",
            }
...
def response_output_text(payload: object) -> str:
    if not isinstance(payload, dict):
        return ""
...
def cmd_configure_desktop(args: argparse.Namespace) -> None:
    ...
    systemd_unit_path = Path(args.systemd_unit).expanduser()
    ...
    if not helper_path.exists():
        emit({"status": "blocked", "error": "the CLIProxyAPI credential helper is missing"}, 2)
    try:
...
def cmd_sync(args: argparse.Namespace) -> None:
    ...
    paths = manifest_paths(selected, state_dir)
    ...
    for path in paths:
        ...
        if errors:
            ...
            manifests_by_slug[data["slug"]] = data
            manifest_sources[data["slug"]] = path
    manifests = list(manifests_by_slug.values())
    if selected:
    ...
    if live_ids:
        available_manifests: list[dict] = []
        for item in manifests:
            slug = item["slug"]
            source = manifest_sources[slug]
            required = bool(selected) or slug in managed_before or source.parent == state_dir / "models.d"
            if slug in live_ids:
                available_manifests.append(item)
            elif required:
...
def probe_prompt(shell: bool, tool_sequence: bool, image_probe: bool = False) -> str:
    ...
    if image_probe:
        return "Inspect the attached image, then reply with exactly: CODEX_BRIDGE_IMAGE_OK"
    return "Reply with exactly: CODEX_BRIDGE_OK"


def probe_png_bytes(width: int = 64, height: int = 64) -> bytes:
    def chunk(kind: bytes, payload: bytes) -> bytes:
        return struct.pack(">I", len(payload)) + kind + payload + struct.pack(">I", zlib.crc32(kind + payload) & 0xFFFFFFFF)

    signature = b"\x89PNG\r\n\x1a\n"
    header = struct.pack(">IIBBBBB", width, height, 8, 2, 0, 0, 0)
    row = b"\x00" + bytes((36, 99, 235)) * width
    pixels = row * height
    return signature + chunk(b"IHDR", header) + chunk(b"IDAT", zlib.compress(pixels, 9)) + chunk(b"IEND", b"")


def redacted_error_tail(value: str, limit: int = 1000) -> str | None:
    if not value.strip():
        return None
    text = re.sub(r"(?i)(authorization:\s*bearer\s+)[^\s]+", r"\1[redacted]", value)
    text = re.sub(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", "[redacted-email]", text)
    text = re.sub(r"\b(?:sk-|AIza)[A-Za-z0-9_-]{12,}\b", "[redacted-key]", text)
...
def replace_toml_table_scalar(text: str, table: str, key: str, value: str) -> str:
    lines = text.splitlines(keepends=True)
    table_pattern = re.compile(rf"^\s*\[{re.escape(table)}\]\s*$")
    key_pattern = re.compile(rf"^\s*{re.escape(key)}\s*=")
    start = next((index for index, line in enumerate(lines) if table_pattern.match(line.rstrip("\r\n"))), None)
    if start is None:
        raise ValueError(f"TOML table is missing: {table}")
    end = next(
...
def isolated_probe_profile(args: argparse.Namespace, catalog: Path, model: str) -> str:
    profile_path = Path(args.profile_config).expanduser() if args.profile_config else DEFAULT_CODEX_HOME / f"{args.profile}.config.toml"
    try:
        text = profile_path.read_text(encoding="utf-8")
        parsed = tomllib.loads(text)
    except (OSError, tomllib.TOMLDecodeError) as exc:
        raise ValueError(f"probe profile is unavailable: {type(exc).__name__}") from exc
    if parsed.get("model_provider") != PROVIDER_ID:
        raise ValueError("probe profile must use cli_proxy command-backed authentication")
    route = provider_from_config(parsed).get("base_url", DEFAULT_PROXY_URL)
    if args.desktop:
        desktop, error = load_config(Path(args.config).expanduser())
        if error:
            raise ValueError(f"desktop config is unavailable: {error}")
        route = desktop.get("openai_base_url")
        if not isinstance(route, str) or not is_loopback(route):
            raise ValueError("desktop probe requires a loopback openai_base_url")
    text = replace_top_scalar(text, "model_provider", PROVIDER_ID)
    text = replace_top_scalar(text, "model_catalog_json", str(catalog))
    text = replace_top_scalar(text, "model", model)
    return replace_toml_table_scalar(text, f"model_providers.{PROVIDER_ID}", "base_url", route)
...
def cmd_probe(args: argparse.Namespace) -> None:
    ...
    probe_config_path = (
        ...
        if args.desktop
        else DEFAULT_CODEX_HOME / f"{args.profile}.config.toml"
    )
    config_sha_before = hashlib.sha256(probe_config_path.read_bytes()).hexdigest() if probe_config_path.exists() else None
    entries = {entry["slug"]: entry for entry in catalog_models(target_path)}
    models = [item.strip() for item in args.models.split(",") if item.strip()]
    if not models:
        emit({"status": "blocked", "error": "--models is required"}, 2)
    if args.image and (args.shell or args.tool_sequence):
        emit({"status": "blocked", "error": "--image cannot be combined with shell probes"}, 2)
    results: dict[str, dict] = {}
    for model in models:
        ...
        if args.fast and "fast" not in entries[model].get("additional_speed_tiers", []):
            ...
            continue
        if args.image and "image" not in entries[model].get("input_modalities", []):
            results[model] = {"ok": False, "error": "model does not advertise image input"}
            continue
        with tempfile.TemporaryDirectory(prefix="codex-model-probe-") as temp_dir:
            temp_codex_home = Path(temp_dir) / "codex-home"
            temp_codex_home.mkdir(mode=0o700)
            probe_profile = temp_codex_home / "probe.config.toml"
            try:
                atomic_write(probe_profile, isolated_probe_profile(args, target_path, model), 0o600)
            ...
            if args.image:
                image_path.write_bytes(probe_png_bytes())
            ...
            command.extend(["--profile", "probe"])
            if args.fast:
                command.extend(["--config", 'service_tier="fast"'])
            if args.shell or args.tool_sequence:
                command.append("--json")
            prompt = probe_prompt(args.shell, args.tool_sequence, args.image)
            command.append(prompt)
            try:
                proc = subprocess.run(
                    command,
                    env={**os.environ, "CODEX_HOME": str(temp_codex_home)},
                ...
                results[model] = {
                    ...
                    "diagnostic": redacted_error_tail(proc.stderr) if not ok else None,
    ...
    config_sha_after = hashlib.sha256(probe_config_path.read_bytes()).hexdigest() if probe_config_path.exists() else None
    config_unchanged = config_sha_before == config_sha_after
    all_passed = all(item["ok"] for item in results.values()) and config_unchanged
    emit(
        {
...
def parser() -> argparse.ArgumentParser:
    ...
    sync.add_argument("--config", default=str(DEFAULT_PROFILE_CONFIG))
    ...
    probe.add_argument("--profile-config")
    ...
    probe.add_argument("--image", action="store_true", help="Attach a generated local PNG and require image input")
    probe.add_argument(
        "--tool-sequence",
        action="store_true",
    ...
    register_deployment_parsers(sub)
    return root
